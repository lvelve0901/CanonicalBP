...
my ($rarray,$refmol)=readPDB("iAformRNA.pdb"); # name of the reference RNA structure
...
if ($order) {
  ...
  foreach my $j (0..length($par1[0])-1) {$Helix1=sprintf "%s\_%s.%s,%s.%d",$Helix1,$chns[0],$res1[$j],'Z',11-length($par1[0])+1+$j;}
  foreach my $j ((length($par2[0]))..(length($par2[0])+length($par2[2])-1)) {$Helix1=sprintf "%s\_%s.%s,%s.%d",$Helix1,$chns[1],$res2[$j],'Z',34-length($par2[0])+$j;}  
  foreach my $j ((length($par1[0]))..(length($par1[0])+length($par1[2])-1)) {$Helix2=sprintf "%s\_%s.%s,%s.%d",$Helix2,$chns[0],$res1[$j],'Z',11-length($par1[0])+1+$j;}
  foreach my $j (0..length($par2[0])-1) {$Helix2=sprintf "%s\_%s.%s,%s.%d",$Helix2,$chns[1],$res2[$j],'Z',34-length($par2[0])+$j;}
  ...
  foreach my $j (0..length($par2[0])-1) {$Helix1=sprintf "%s\_%s.%s,%s.%d",$Helix1,$chns[1],$res2[$j],'Z',11-length($par2[0])+1+$j;}
  foreach my $j ((length($par1[0]))..(length($par1[0])+length($par1[2])-1)) {$Helix1=sprintf "%s\_%s.%s,%s.%d",$Helix1,$chns[0],$res1[$j],'Z',34-length($par1[0])+$j;}  
  foreach my $j ((length($par2[0]))..(length($par2[0])+length($par2[2])-1)) {$Helix2=sprintf "%s\_%s.%s,%s.%d",$Helix2,$chns[1],$res2[$j],'Z',11-length($par2[0])+1+$j;}
  foreach my $j (0..length($par1[0])-1) {$Helix2=sprintf "%s\_%s.%s,%s.%d",$Helix2,$chns[0],$res1[$j],'Z',34-length($par1[0])+$j;} 
  #print $Helix1,"\n";
