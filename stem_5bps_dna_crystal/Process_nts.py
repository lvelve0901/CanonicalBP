...
for idx, pdb in enumerate(pdblist):
    ...
    if os.path.isfile(json_f):
        pass
    elif os.path.isfile(pdb_f):  #DSSR convert PDB to json files
    ...
    for bp in bps:
        if (bp['nt1'] == purine['nt_id'] and bp['nt2'] == pyrimi['nt_id']) or (bp['nt2'] == purine['nt_id'] and bp['nt1'] == pyrimi['nt_id']):
            ct_bp = bp
            break

    if len(ct_bp) == 0:
        print("ERROR(): counld not assign center bp")
        sys.exit()
